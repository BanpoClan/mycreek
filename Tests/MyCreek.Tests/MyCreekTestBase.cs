...
namespace MyCreek.Tests
{
    public abstract class MyCreekTestBase : AbpIntegratedTestBase<MyCreekTestModule>
    {
        ...
        protected void UsingDbContext(Action<MyCreekDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<MyCreekDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<MyCreekDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<MyCreekDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<MyCreekDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MyCreekDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<MyCreekDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MyCreekDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<MyCreekDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<MyCreekDbContext, Task<T>> func)
        {
            T result;
